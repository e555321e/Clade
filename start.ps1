...
# 启动后端 (PowerShell 窗口)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$scriptDir\backend'; .\venv\Scripts\Activate.ps1; Write-Host 'Clade 后端服务' -ForegroundColor Green; python -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000"
...
# 启动前端 (PowerShell 窗口)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$scriptDir\frontend'; Write-Host 'Clade 前端服务' -ForegroundColor Green; npm run dev"
